<script lang="ts">
	...
	const content = $derived(privacyContent[locale]);
	const siteUrl = publicConfig.siteUrl ?? 'http://localhost';
	const canonicalHref = $derived(new URL(localizeHref('/privacy', { locale }), siteUrl).href);
	const frenchHref = $derived(new URL(
		localizeHref('/privacy', { locale: 'fr' }),
		siteUrl
	).href);
	const englishHref = $derived(new URL(
		localizeHref('/privacy', { locale: 'en' }),
		siteUrl
	).href);
